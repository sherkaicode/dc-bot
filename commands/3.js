module.exports = {
    ...
    async execute(message, args, Discord, client, fs) {
        let _message = client.msgs3[message.author.username].message;
        ...
        let embed = new Discord.MessageEmbed()
            .setColor('#e42643')
            .setTitle('三')
            .setURL('https://jisho.org/search/%E4%B8%89%23kanji')
            .setDescription('[さん]')
            .setThumbnail('https://raw.githubusercontent.com/mistval/kanji_images/master/gifs/4e09.gif')
            .addFields(
                { name: 'English', value: 'three' },
                        ...
                        message.channel.awaitMessages(m => m.author.id == message.author.id,
                            { max: 1, time: 30000 }).then(collected => {
                                ...
                                if (collected.first().content.toLowerCase()) {
                                    ...
                                    client.msgs3[message.author.username] = {
                                        message: mes
                                    }
                                    fs.writeFile("./Database/3.json", JSON.stringify(client.msgs3, null, 4), err => {
                                        if (err) throw err;
                                    })

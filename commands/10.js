module.exports = {
    ...
    async execute(message, args, Discord, client, fs) {
        let _message = client.msgs10[message.author.username].message;
        ...
        let embed = new Discord.MessageEmbed()
            .setColor('#e42643')
            .setTitle('十')
            .setURL('https://jisho.org/search/%E5%8D%81%23kanji')
            .setDescription('[じゅう]')
            .setThumbnail('https://raw.githubusercontent.com/mistval/kanji_images/master/gifs/5341.gif')
            .addFields(
                { name: 'English', value: 'Ten' },
                        ...
                        message.channel.awaitMessages(m => m.author.id == message.author.id,
                            { max: 1, time: 30000 }).then(collected => {
                                ...
                                if (collected.first().content.toLowerCase()) {
                                    ...
                                    client.msgs10[message.author.username] = {
                                        message: mes
                                    }
                                    fs.writeFile("./Database/10.json", JSON.stringify(client.msgs10, null, 4), err => {
                                        if (err) throw err;
                                    })

module.exports = {
    ...
    async execute(message, args, Discord, client, fs) {
        let _message = client.msgs13[message.author.username].message;
        ...
        let embed = new Discord.MessageEmbed()
            .setColor('#e42643')
            .setTitle('月')
            .setURL('https://jisho.org/search/%E6%9C%88%23kanji')
            .setDescription('[つき]')
            .setThumbnail('https://raw.githubusercontent.com/mistval/kanji_images/master/gifs/6708.gif')
            .addFields(
                { name: 'English', value: 'Month' },
                        ...
                        message.channel.awaitMessages(m => m.author.id == message.author.id,
                            { max: 1, time: 30000 }).then(collected => {
                                ...
                                if (collected.first().content.toLowerCase()) {
                                    ...
                                    client.msgs13[message.author.username] = {
                                        message: mes
                                    }
                                    fs.writeFile("./Database/13.json", JSON.stringify(client.msgs13, null, 4), err => {
                                        if (err) throw err;
                                    })

module.exports = {
    ...
    async execute(message, args, Discord, client, fs) {
        let _message = client.msgs1[message.author.username].message;
                        ...
                        message.channel.awaitMessages(m => m.author.id == message.author.id,
                            { max: 1, time: 30000 }).then(collected => {
                                ...
                                if (collected.first().content.toLowerCase()) {
                                    ...
                                    client.msgs1[message.author.username] = {
                                        message: mes
                                    }
                                    fs.writeFile("./Database/1.json", JSON.stringify(client.msgs1, null, 4), err => {
                                        if (err) throw err;
                                    })
